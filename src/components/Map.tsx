  ...
  return (
    <div className="relative w-full h-[320px] md:h-[400px] bg-card rounded-2xl border border-stone-200 overflow-hidden shadow-inner">
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none opacity-40">
        {/* Gran Via */}
        <path d="M 10,130 Q 150,150 350,180 T 600,210" fill="none" stroke="var(--primary)" strokeWidth="8" strokeLinecap="round" />
        <text x="120" y="135" fill="var(--muted-foreground)" className="text-[10px] font-semibold font-sans uppercase tracking-wider">Gran Vía</text>
        
        {/* Paseo del Prado */}
        <path d="M 450,10 Q 500,200 550,390" fill="none" stroke="var(--primary)" strokeWidth="10" strokeLinecap="round" />
        <text x="475" y="160" fill="var(--muted-foreground)" className="text-[10px] font-semibold font-sans uppercase tracking-wider rotate-85" style={{ transformOrigin: "475px 160px" }}>Paseo del Prado</text>

        {/* Calle de Alcalá */}
        <path d="M 300,170 Q 420,150 550,140 T 750,130" fill="none" stroke="var(--primary)" strokeWidth="6" strokeLinecap="round" />
        <text x="360" y="152" fill="var(--muted-foreground)" className="text-[10px] font-semibold font-sans uppercase tracking-wider">Calle de Alcalá</text>

        {/* Calle de Atocha */}
        <path d="M 280,240 Q 380,270 510,310 T 600,350" fill="none" stroke="var(--primary)" strokeWidth="5" strokeLinecap="round" />
        <text x="340" y="270" fill="var(--muted-foreground)" className="text-[10px] font-semibold font-sans uppercase tracking-wider">Calle de Atocha</text>
      </svg>
      ...
      {MADRID_MAP_SPOTS.map((spot) => {
        ...
        return (
          ...
          >
            ...
            {isHighlighted && !isSelected && (
              <div className="absolute -top-7 left-1/2 transform -translate-x-1/2 bg-amber-50 text-amber-800 font-mono text-[8px] font-extrabold px-1.5 py-0.5 rounded shadow border border-amber-200 uppercase tracking-widest whitespace-nowrap">
                Shared Interest
              </div>
            )}
          </button>
