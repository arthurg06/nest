...
- **Account recovery**: "Forgot your password?" on the login screen issues a single-use link (SHA-256 digest stored, one hour, invalidates earlier links, clears every session on use). Email delivery is optional — with `RESEND_API_KEY` + `EMAIL_FROM` the link is emailed; without them an admin generates the same link from the member list and passes it on directly, which is the path that works today.
- **Profiles**: up to 6 photos per member (`photos[]`, primary first; the legacy `photo` field always mirrors `photos[0]` so older records and clients keep working). Interests include a single-choice **animals** preference (dog lover / cat lover / loves all animals / plants over pets), which feeds compatibility. Sign-up and profile updates both run the same server-side sanitizer: unknown animal values are dropped, photos are capped at 6 and must be app-issued or https URLs.
